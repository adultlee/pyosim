...
def test_parse_20th_returns_tuple():
    rows, totals = parse_20th()
    assert isinstance(rows, list)
    assert isinstance(totals, list)


def test_parse_20th_row_columns():
    rows, _ = parse_20th()
    assert len(rows) > 0
...
def test_parse_20th_round():
    rows, _ = parse_20th()
    assert all(r["선거_회차"] == "제20대" for r in rows)


def test_parse_20th_선거구분():
    rows, _ = parse_20th()
...
def test_parse_20th_passes_validation():
    rows, totals = parse_20th()
    df = to_dataframe(rows)
    official = {k: v for k, v in OFFICIAL_TOP.items() if k[0] == "제20대"}
