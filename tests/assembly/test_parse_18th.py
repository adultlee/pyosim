...
def test_parse_18th_returns_tuple():
    rows, totals = parse_18th()
    assert isinstance(rows, list)
    assert isinstance(totals, list)


def test_parse_18th_row_columns():
    rows, _ = parse_18th()
    assert len(rows) > 0
...
def test_parse_18th_round():
    rows, _ = parse_18th()
    assert all(r["선거_회차"] == "제18대" for r in rows)


def test_parse_18th_선거구분():
    rows, _ = parse_18th()
...
def test_parse_18th_passes_validation():
    rows, totals = parse_18th()
    df = to_dataframe(rows)
    official = {k: v for k, v in OFFICIAL_TOP.items() if k[0] == "제18대"}
