...
ELECTION_DATE = "1997-12-18"


def test_parse_15_returns_rows_and_totals():
    rows, totals = parse_15()
    assert len(rows) > 0
...
def test_parse_15_no_nan_sido():
    rows, _ = parse_15()
    df = to_dataframe(rows)
    assert df["시도"].isna().sum() == 0


def test_parse_15_passes_all_validation():
    rows, totals = parse_15()
    for row in rows:
        row.setdefault("선거일", ELECTION_DATE)
    df = to_dataframe(rows)
    official = {k: v for k, v in OFFICIAL_TOP.items() if k[0] == 15}
